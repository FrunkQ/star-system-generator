...
### 7c. DOCKING - the owner's requirement, 2026-09-03 (SHIPPED v3.0.298; arrival sense + catch hand-over v3.0.324 as recommended below - engine map RENDER-S51; `constructs/docking.ts` is the module, `worldPositions.docking.spec.ts` and `dockingArrival.spec.ts` the gates)
