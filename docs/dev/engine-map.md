...
### UI-C18 A DOCKED GROUP IS ONE BOX WITH ONE EDGE, AND ONLY ONE SETTLE MAY RUN PER MOVE
BUCKET: ARCHITECTURE - the shape carries (a registry, a group id per member, the union as the settled box); the 12/24 px are implementation.
WHERE: `src/lib/ui/floatingControl.ts` (`members`, `unionOf`, `apart`, `groupMembers`, `redock`, and `settle`'s union), pinned by `floatingControl.spec.ts` "two controls dock, move as one".
RULE: controls are still INDEPENDENT - a host knows nothing of any other host - and docking is the one thing they share: each registers itself in a module-level set while its root action is mounted, and a group is every registered control with the same `dock` id ON THE SAME STAGE. What settles is the UNION of the group's rects, and the correction is applied to every member as the SAME delta; every member then stores the SAME `ex/gx/ey/gy`, which is the group's and not its own. An undocked control is a group of one and the identical code runs - there is no second path.
WHY: G81, the owner's *"allow them to be pinned together"*. Settling members SEPARATELY looks right and is not: each is correct for itself and the pair drifts, because the gap that matters belongs to the union. Two hidden consequences, both paid for once. (1) **A control leaving a group must re-read its own edge and gap** (`settle(false)`), or it keeps the union's - which for the member that was NOT at the union's edge means inheriting its neighbour's distance from the wall, and the two then settle on top of each other. (2) **A gate that only checks "did both move together" passes with the union removed**, because member A settles, nudges B, then B settles and nudges A back: the pair lands in the right place by accident. Only a case where the clamp bites on the UNION's width rather than on either member's tells them apart - 100 against 196, in the gate.
BLAST: **ONE settle per move, and it must not be one per member.** A group drag calls `nudge` on the others - `set(..., false)` with no re-settle - because a settle per member per frame is exactly the fight UI-C17 warns about; the same goes for the resize observers, which every member has: the first to fire settles the group and the rest then compute a delta of zero. **`redock` runs on RELEASE and never per frame**, or a control dragged past a neighbour snaps to it in passing. **The 24 px is DRIFT, not a drag-away gesture:** with one grip moving all, a docked control cannot be dragged away from its group, so the only way a member gets 24 px clear is by something changing SIZE - which the undo pill and the picker both do. Undocking is a menu item.
