<script lang="ts">
  ...
  function rendered(s: Subject, bodySize: number): number {
    const ctx = { bodySize, rMax };
  ...
  function violates(rows: Subject[], i: number, dial: number): boolean {
    if (i === 0) return false;
    return rendered(rows[i], dial) > rendered(rows[i - 1], dial) * (1 + 1e-9);
  }
...
<main>
  ...
  <div class="verdict" class:bad={violationCount > 0}>
    {#if violationCount > 0}
      ...
      NEVER renders smaller than a smaller one. Flagged cells below are where today's law breaks that —
      the construct band (0.14–0.7) overlaps the body band, so small ships out-draw real moons.
      <em>This is expected before phase P4</em>; it is the definition of done for the new law, and the
      matching test is <code>describe.skip('R9 ordering')</code> in the spec.
    {:else}
      <strong>No ordering violations.</strong> Every column is non-increasing down the table, at every
      dial position and system extent — R9 holds. Un-skip the R9 block in the spec.
    {/if}
  </div>
