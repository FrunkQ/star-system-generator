...
vec3 processColor(vec3 aberrColor, vec2 screenCoord, vec2 baseUv, bool isInsideNoiseBar, float posInBar) {
    ...
    // Amber tint applied early — warms the image before brightness/contrast
    if (uAmberTint > 0.0) {
        float t = uAmberTint;
        processed = vec3(
            processed.r,
            processed.g * (1.0 - t * 0.35),
            processed.b * (1.0 - t * 0.9)
        );
    }
