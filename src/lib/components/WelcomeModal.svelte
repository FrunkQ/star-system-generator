<script lang="ts">
  ...
  const features: { title: string; blurb: string; pending?: string }[] = [
    ...
    { title: 'Stand on the surface', blurb: 'A world has a Horizon view: the landscape as it would look under that world&rsquo;s own daylight, with the sky and the ground taking their colour from the light that actually reaches them. A red dwarf&rsquo;s noon does not look like ours — and the engine answers how far your players can see, and how far a lamp carries, in metres rather than in adjectives.' },
