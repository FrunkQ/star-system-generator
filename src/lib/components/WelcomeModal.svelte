<script lang="ts">
  ...
  const features: { title: string; blurb: string; pending?: string }[] = [
    ...
    { title: 'Stars rebuilt from the physics up', blurb: 'A star’s type is now read from where it sits — how big it is for its temperature — rather than from how bright it looks, which is what tells a brilliant blue dwarf from a supergiant. Brightness follows from size and heat instead of being stored, so it cannot drift. And brightness is no longer confused with the radiation that strips atmospheres: that follows a star’s magnetic field, so winding the field up drives a star into flaring — until it saturates, which the panel says, because real stars have a ceiling. Cool giants correctly stop being fierce X-ray sources. Every type is named in plain words with a famous example, and a star whose numbers break a law of physics is kept and labelled rather than refused.' },
