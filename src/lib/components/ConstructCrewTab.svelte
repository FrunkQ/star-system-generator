<script lang="ts">
  ...
  import { apparentGravity } from '$lib/constructs/apparentG';

  export let construct: CelestialBody;
  /** The construct's host, for an exotic's apparent gravity (net of the host's pull). */
  ...
  onMount(() => {
    // G58 N2: an exotic's gravity is its RECORD's business (apparentG.ts) - no station-shaped
    // field is written onto it. That default was the shim behind the 12,967,908 g ringworld.
    if (apparentGravity(construct, hostBody)) return;
  ...
  // G58 N2 - THE LAST SEAM: an exotic's apparent gravity comes from its record's declared wiring
  // (constructs/apparentG.ts): the ring's own rotation NET OF ITS HOST (zero at orbital rate), a
  // hull's own mass, or honestly nothing. The owner's decision is his own three-wirings sentence.
  $: exoticG = apparentGravity(construct, hostBody);
...
<div class="tab-panel">
  ...
  {#if exoticG}
    <!-- G58 N2: an exotic's apparent gravity is what its RECORD declares (constructs/apparentG.ts):
         the ring's own rotation net of its host, a hull's own mass, or honestly nothing. The station
         spin section is not its business, and no station-shaped field is written onto it. -->
    <h4>Apparent Gravity{#if exoticG.wiring !== 'none'} (Current: {(exoticG.ms2 / 9.80665).toFixed(2)} g){/if}</h4>
    <p style="font-size:0.85em; opacity:0.85; margin:0.2em 0 0.6em; line-height:1.35;">{exoticG.note}</p>
  {:else}
...
<style>
  ...
  input[type="number"], select, textarea { padding: 8px; border-radius: 4px; border: 1px solid var(--border); background-color: var(--bg-control); color: var(--text); font-size: 1em; width: 100%; box-sizing: border-box; }
