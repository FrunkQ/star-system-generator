...
import { sniffBundle, unpackBundle } from './bundle';
...
export function classifySaveFile(raw: Uint8Array): ClassifiedSaveFile {
  if (sniffBundle(raw)) {
    try {
      const { kind, doc, models } = unpackBundle(raw);
      return { kind, container: 'bundle', doc, models };
    } catch (e) {
  ...
  return { kind, container: 'json', doc };
}
